...
public class AnalyticsReceiver {
	...
	@JmsListener(destination = JMSTopics.Analytics.ANALYSE_STANDALONE_DESCRITOR_POPULATION+ JMSTopics.IN)
	public void analyseStandaloneDescriptorPopulation(String workflowId){
		try {
			log.info(ANALYTICS_RECEIVED_MESSAGE, JMSTopics.Analytics.ANALYSE_STANDALONE_DESCRITOR_POPULATION,workflowId);
			this.analyticsService.processStandaloneDescriptorPopulation(workflowId);
			this.sender.send(JMSTopics.Analytics.ANALYSE_STANDALONE_DESCRITOR_POPULATION+ JMSTopics.OUT, workflowId);
		}catch(Exception e) {
			log.error(e.toString(),e);
			this.sender.send(JMSTopics.Analytics.ANALYSE_STANDALONE_DESCRITOR_POPULATION+ JMSTopics.ERROR, workflowId);
		}
	}

	@JmsListener(destination = JMSTopics.Analytics.ANALYSE_JOINED_DESCRITOR_POPULATION+ JMSTopics.IN)
	public void analyseJoinedDescriptorPopulation(String workflowId){
		...
		try {
			...
			log.info(ANALYTICS_RECEIVED_MESSAGE, JMSTopics.Analytics.ANALYSE_JOINED_DESCRITOR_POPULATION,workflowId);
			this.analyticsService.processJoinedDescriptorPopulation(workflowId);
			this.sender.send(JMSTopics.Analytics.ANALYSE_JOINED_DESCRITOR_POPULATION+ JMSTopics.OUT, workflowId);
		}catch(Exception e) {
			log.error(e.toString(),e);
			if(dwdp != null){
			...
			this.sender.send(JMSTopics.Analytics.ANALYSE_JOINED_DESCRITOR_POPULATION+ JMSTopics.ERROR, workflowId);
		}
	}


	@JmsListener(destination = JMSTopics.Analytics.ANALYSE_JOINER_DESCRITOR_POPULATION+ JMSTopics.IN)
    public void analyseJoinerDescriptorPopulation(String workflowId){
        DescriptorWorkflowDataPackage dwdp = null;
        try {
            dwdp = this.dwdpDAO.get(workflowId);
            log.info(ANALYTICS_RECEIVED_MESSAGE, JMSTopics.Analytics.ANALYSE_JOINER_DESCRITOR_POPULATION,workflowId);
            this.analyticsService.processJoinerDescriptorPopulation(workflowId);
            this.sender.send(JMSTopics.Analytics.ANALYSE_JOINER_DESCRITOR_POPULATION+ JMSTopics.OUT, workflowId);
        }catch(Exception e) {
            log.error(e.toString(),e);
            if(dwdp != null){
                dwdp.getDebugInformation().setException(e.getMessage()+"\n"+ Arrays.stream(e.getStackTrace()).map(StackTraceElement::toString).collect(Collectors.joining("\n")));
            ...
            this.sender.send(JMSTopics.Analytics.ANALYSE_JOINER_DESCRITOR_POPULATION+ JMSTopics.ERROR, workflowId);
        }
    }
